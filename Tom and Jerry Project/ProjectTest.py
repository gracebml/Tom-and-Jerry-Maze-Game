...
# bắt đầu vào game 
game_clock.start()
def reset_game(grid_cells, TILE,drawer, start, end):
    ...
    while True:
        ...
        if target_cell == end:
                ...
                if option_after_game:
                    for i in range(2):
                        sound.toggle_sound()
                    grid_cells,drawer, start, end = new_game(grid_cells,TILE,drawer)
        ...
        if int(check_time_up) > 15*60: #Chơi lâu hơn 15p là thua
            playSound(5)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:  # Tạm dừng trò chơi
                    ...
                    grid_cells,drawer, start, end = reset_game(grid_cells,TILE,drawer, start, end)
                    # reset đồng hồ
                    game_clock.start()
                    player.x, player.y = start.x, start.y
                    player_index = player.x + player.y * cols
