...
def draw_dashboard(steps, level, mode_game, algo):
    ...
    time_text = d_font.render(f"{hours:02}:{minutes:02}:{seconds:02}", True, "red")
    screen.blit(time_text, time_coord2)
    # vẽ step
    screen.blit(d_font.render('Steps:', True, pygame.Color("yellow")), step_coord1)
    screen.blit(d_font.render(f'{steps}', True, pygame.Color("yellow")), step_coord2)
    # vẽ mode game
    screen.blit(d_font.render('Mode:', True, pygame.Color("yellow")), mode_coord1)
    screen.blit(d_font.render(f'{mode_game}', True, pygame.Color("yellow")), mode_coord2)
    # vẽ level
    screen.blit(d_font.render('Level: ', True, pygame.Color("yellow")), level_coord1)
    screen.blit(d_font.render(f'{level}', True, pygame.Color("yellow")), level_coord2)
    # vẽ thuật toán hiện tại
    screen.blit(d_font.render('Algorithm:', True, pygame.Color("yellow")), algo_coord1)
    screen.blit(d_font.render(f'{algo}', True, pygame.Color("yellow")), algo_coord2)
